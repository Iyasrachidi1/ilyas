...
# URL du fichier GPKG dans le dépôt GitHub (raw content)
gpkg_url = "https://raw.githubusercontent.com/Iyasrachidi1/ilyas/master/donnes.gpkg"

# Télécharger le fichier GPKG à la volée
response = requests.get(gpkg_url)
if response.status_code == 200:
    st.sidebar.write("Fichier téléchargé avec succès depuis le dépôt GitHub.")
    gpkg_data = BytesIO(response.content)
else:
    st.sidebar.error("Erreur lors du téléchargement du fichier GPKG.")
    st.stop()

# Charger la seule couche disponible dans le GPKG
gdf = gpd.read_file(gpkg_data)
